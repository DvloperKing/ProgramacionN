...
class SaveImageToFileWorker(ctx: Context, params: WorkerParameters) : CoroutineWorker(ctx, params) {
    ...
    override suspend fun doWork(): Result {
        makeStatusNotification(applicationContext.resources.getString(R.string.saving_image),
                               applicationContext)
        return withContext(Dispatchers.IO) {
            ...
            return@withContext try {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                    ...
                    val imageUri = createImageUriUseCase(resolver,
                                                 "${title}_${dateFormatter.format(Date())}.jpg")
                    saveImageUseCase(resolver, imageUri, bitmap)
                    imageUrl = imageUri.toString()
                } else {
                    @Suppress("DEPRECATION")
                    imageUrl = MediaStore.Images.Media.insertImage(resolver, bitmap, title, dateFormatter.format(Date()))
                }
