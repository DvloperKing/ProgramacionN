...
@Composable fun BluromaticScreen(blurViewModel: BlurViewModel = viewModel(factory = BlurViewModel.Factory)) {
    ...
    Surface(modifier = Modifier.fillMaxSize()
        ...
        BluromaticScreenContent(blurUiState = uiState,
...
@Composable fun BluromaticScreenContent(blurUiState: BlurUiState,
    ...
    Column(modifier = modifier) {
        ...
        BlurActions(blurUiState = blurUiState,
...
@Composable private fun BlurActions(blurUiState: BlurUiState,
                                    onStartClick: () -> Unit,
...
@Composable private fun BlurAmountContent(selectedValue: Int,
                                          blurAmounts: List<BlurAmount>,
                                          modifier: Modifier = Modifier,
                                          onSelectedValueChange: (Int) -> Unit) {
    Column(modifier = modifier.selectableGroup()) {
        Text(text = stringResource(R.string.blur_title),
        ...
        blurAmounts.forEach { amount ->
            Row(verticalAlignment = Alignment.CenterVertically,
                modifier = Modifier.fillMaxWidth()
                                   ...
                                   .size(48.dp)) {
                RadioButton(selected = (selectedValue == amount.blurAmount),
                            onClick = null,
...
private fun showBlurredImage(context: Context, currentUri: String) {
    val uri = if (currentUri.isNotEmpty()) { Uri.parse(currentUri) }
    else { null }
